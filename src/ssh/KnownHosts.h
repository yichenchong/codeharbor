...
// directly. Hashed (|1|salt|hash) host entries are preserved on round-trip but
// are opaque: they never produce a Match/Mismatch (supported == false).
class KnownHosts {
public:
    enum class Verdict {
    ...
    // Unknown if the host+keyType pair is absent. Hashed entries never match.
