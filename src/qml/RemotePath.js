...
// remote file service speaks (SPEC 8.3): strip the "file://" scheme, then
// percent-decode.
//
...
// place: ViewerPane, ViewerDirectoryView, ViewerMarkdownView and EditorPaneView
// all call here instead of each re-implementing the rule.
function fileUrlToPath(url) {
    ...
    var s = String(url);
    if (s.indexOf("file://") === 0) {
        var encoded = s.substring("file://".length);
        try {
            return decodeURIComponent(encoded);
        } catch (error) {
            // A hand-entered address can contain a malformed percent escape.
            // Keep the raw server path rather than throwing during a binding
            // evaluation and taking the whole pane out of service.
            return encoded;
        }
    }
    return s;
